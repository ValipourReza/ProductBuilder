...
public class ProductController {
    ...
    @GetMapping("/products/{id}") public String detail(@PathVariable Long id,Model m){Product p=products.findByIdWithFeatures(id).orElseThrow();m.addAttribute("product",p);m.addAttribute("features",features.findByActiveTrueOrderByNameAsc());m.addAttribute("statuses",ProductStatus.values());return "product-detail";}
